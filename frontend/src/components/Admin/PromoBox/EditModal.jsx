...
const EditModal = ({ show, setShow, promoData, setPromoData }) => {
    ...
    try {
      ...
      data.length > 0 &&
        data.forEach(async (x, i) => {
          const res = await userRequest.post("/promotion_1/add", {
            imageFile: x.img,
      ...
      setLoading(false);
      alert("Promo Successfully uploaded");
      setShow(false);
    } catch (error) {
      alert(error.message);
      setLoading(false);
